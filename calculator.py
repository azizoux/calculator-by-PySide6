...
class Calculator(QWidget):
    def __init__(self):
        ...
        for button_txt, button_position in BUTTONS.items():
            ...
            button.setStyleSheet(f"""
                QPushButton {{
                    border: none;
                    font-weight: bold;
                    background-color:{'#1e1e2d' if button_txt in OPERATIONS else 'none' };
                }}
                QPushButton:pressed {{background-color: #f31d58;}}
            """)

            if button_txt not in ["=", "C"]:
                button.clicked.connect(self.number_or_operations_pressed)
            self.buttons[button_txt] = button
